package com.codingmonster.marketdata;
...
public class MarketDataAgent implements Agent {
  ...
  public MarketDataAgent(
      Aeron aeron, String pubChannel, int pubStreamId, String subChannel, int subStreamId) {
    this.aeron = aeron;
  ...
  private int handleNewOrder() {
    ...
    NewOrderMessageEncoder order = new NewOrderMessageEncoder();
    order
    ...
    order.price().mantissa(12345).exponent((byte) -3);
    int len = headerEncoder.encodedLength() + order.encodedLength();
    long result;
    while (true) {
      result = publication.offer(sendBuffer, 0, len);
      if (result > 0) {
        break;
      } else {
        handleFailure(result, idleStrategy);
      }
    }
  ...
  private void handleFailure(long result, IdleStrategy idle) {
